...
describe('display name contract', () => {
  it('defines an explicit exact-match displayName assignment for every public component', () => {
    ...
    const offenders = publicComponentNames.filter((componentName) => {
      const displayNamePattern = new RegExp(`\\b${componentName}\\.displayName\\s*=\\s*['"]${componentName}['"]`);

      return !sources.some((source) => displayNamePattern.test(source));
    });
