...
- Local directories to inspect first when relevant: `docs/`, `scripts/`, `specs/`, `src/`.
...
The `build/` directory in this package contains git-tracked build-critical source contracts (`package-contract.ts`, `docs-governance-contract.ts`, `generated-reference-contract.json`). These files are imported by `vite.config.ts` at config load time and are not generated artifacts. They must not be deleted by `clean`, cache purges, or `.gitignore`. Follow `CODE_STYLE_SPEC.md` §7 for build source integrity and self-healing rules.
